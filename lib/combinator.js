...
function Combinator(config) {
    ...
    this._regexes = {
        ...
        filename  : new RegExp(this.options.filter || ".*")
    };
}
...
Combinator.prototype = {
    ...
    _findNodes : function(dom) {
        ...
        traverse(dom).forEach(function() {
            ...
            if(this.node.type === "tag" && this.node.attribs && this.node.attribs.rel === "stylesheet") {
                css.push(this.path);
            }
        });
    ...
    transformFile : function(args) {
        ...
        args.done(null, { file : args.file, text : args.text });
    },
    ...
    run : function() {
        ...
        async.map(
            ...
            function(error, results) {
                ...
                console.log(util.inspect(results, null, null, true)); //TODO: REMOVE DEBUGGING
            }
        );
