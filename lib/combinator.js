...
Combinator.prototype = {
    ...
    _findNodes : function(dom) {
        ...
        traverse(dom).forEach(function() {
            ...
            //check for <script>
            if(self._validScriptNode(node) && self._regexes.srcpath.test(node.attribs.src)) {
                js.push(this.path);
            }
            
            //check for <link rel="stylesheet" />
            if(self._validLinkNode(node) && self._regexes.srcpath.test(node.attribs.href)) {
                css.push(this.path);
            }
        });
    ...
    _groupNodes : function(paths, dom) {
        ...
        paths.forEach(function(path) {
            ...
            console.log(uri); //TODO: REMOVE DEBUGGING
            ...
            console.log(group); //TODO: REMOVE DEBUGGING
        ...
        _lib.objEach(groups, function(details, group) {
            ...
            paths = paths.filter(function(curr, idx) {
                ...
                // First item needs to check the next value, instead of everyone else who checks previous
                ...
                text = tdom.get(details.root.concat(typeof text !== "undefined" ? text : curr - 1));
            ...
            _lib.objEach(groups, function(details) {
                ...
                details.paths.forEach(function(element) {
                    ...
                    if(link.indexOf(self.options.combo) > -1) {
                        uri = uriparser(link.replace(self.options.combo, ""));
                        uri = uri.parse(link.replace(self.options.combo, ""));
                    }
                    
                    output.push(uri.pathname || uri.href);
